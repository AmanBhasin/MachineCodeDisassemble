...
void InsertNops(vector<Instruction> &instructions)
{
    ...
    if (instructions[0].insType != "sd")
    {
        if (instructions[0].dest != "")
            whenFree[instructions[0].dest] = 5;
    ...
    {
        whenFree[instructions[0].dest] = 2;
        int pos = instructions[0].src1.size() - 2;

        whenFree[(instructions[0].src1).substr(pos)] = 5;
    }
